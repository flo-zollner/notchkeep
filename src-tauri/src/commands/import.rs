...
pub async fn import_trade_republic_csv(
    ...
    let parsed = TradeRepublicCsv.parse(&bytes).map_err(|e| CommandError {
        message: e.to_string(),
    })?;
    let pool = state.pool();
...
pub async fn import_flatex_pdfs(
    ...
    let effective_account_id =
        crate::db::accounts::resolve_cash_settlement_account(&pool, account_id)
            .await
            .ok()
            .flatten()
    ...
    for bytes in &files {
        let hash = source_file_hash(bytes);
        let parsed = FlatexPdf.parse(bytes).map_err(|e| CommandError {
            message: e.to_string(),
        })?;
...
mod tests {
    ...
    async fn ensure_tr_institution_creates_if_missing() {
        ...
        let acc = crate::db::accounts::create_account(&pool, "TR", "bank", "EUR", None, None, None)
            .await
            .unwrap();
        super::ensure_tr_institution_for_account(&pool, acc.id)
            .await
            .unwrap();
        ...
        let tr = get_institution_by_name(&pool, "Trade Republic")
            .await
            .unwrap();
        assert!(tr.is_some());
    }
    ...
    async fn ensure_flatex_institution_creates_if_missing() {
        ...
        .await
        .unwrap();
        super::ensure_flatex_institution_for_account(&pool, acc.id)
            .await
            .unwrap();
        let reloaded = crate::db::accounts::get_account(&pool, acc.id)
            .await
            .unwrap();
        assert!(reloaded.institution_id.is_some());
        let inst = crate::db::institutions::get_institution_by_name(&pool, "flatexDEGIRO")
            .await
            .unwrap();
        assert!(inst.is_some());
    }
    ...
    async fn filter_sparplan_skips_duplicate_when_einzelbeleg_already_imported() {
        ...
        sqlx::query(
            "INSERT INTO securities (isin, name, asset_type, currency)
        ...
        .unwrap();
        let (sec_id,): (i64,) =
            sqlx::query_as("SELECT id FROM securities WHERE isin='LU1781541179'")
                .fetch_one(&pool)
                .await
                .unwrap();
        ...
        let (tx_id,): (i64,) =
            sqlx::query_as("SELECT id FROM transactions WHERE raw_ref='195830560/1'")
                .fetch_one(&pool)
                .await
                .unwrap();
        ...
        let (filtered, skipped) =
            super::filter_sparplan_duplicates(&pool, acc.id, vec![sp_dup, sp_new])
                .await
                .unwrap();
        assert_eq!(
            skipped, 1,
            "SP Tx with same date+amount+ISIN should be skipped"
        );
        assert_eq!(
            filtered.len(),
            1,
            "new SP Tx (different day) should pass through"
        );
    ...
    async fn displace_sparplan_removes_existing_sp_tx_when_einzelbeleg_arrives() {
        ...
        sqlx::query("INSERT INTO transactions (account_id, booking_date, amount_cents, currency, counterparty, source, kind, raw_ref)
            ...
            .bind(acc.id).execute(&pool).await.unwrap();
        let (sp_tx_id,): (i64,) =
            sqlx::query_as("SELECT id FROM transactions WHERE raw_ref='SP0001818664-0'")
                .fetch_one(&pool)
                .await
                .unwrap();
        sqlx::query("INSERT INTO securities_trades
        ...
        let (sp_remaining,): (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM transactions WHERE raw_ref = 'SP0001818664-0'")
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(sp_remaining, 0, "SP Tx should be deleted");
        let (st_remaining,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM securities_trades")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert_eq!(
            st_remaining, 0,
            "associated securities_trades should be gone via CASCADE"
        );
    }
    ...
    async fn displace_sparplan_ignores_non_sp_existing_tx() {
        ...
        assert_eq!(
            deleted, 0,
            "displace must only delete SP Tx, not regular ones"
        );
    ...
    async fn filter_sparplan_keeps_non_sp_tx_untouched() {
        ...
        let (filtered, skipped) = super::filter_sparplan_duplicates(&pool, acc.id, vec![plain])
            .await
            .unwrap();
        assert_eq!(skipped, 0);
    ...
    async fn ensure_sparkasse_institution_creates_if_missing() {
        ...
        .await
        .unwrap();
        super::ensure_sparkasse_institution_for_account(&pool, acc.id)
            .await
            .unwrap();
        let reloaded = crate::db::accounts::get_account(&pool, acc.id)
            .await
        ...
        assert!(reloaded.institution_id.is_some());
        let inst =
            crate::db::institutions::get_institution_by_name(&pool, "Erste Bank / Sparkasse")
                .await
                .unwrap();
        assert!(inst.is_some());
    }
