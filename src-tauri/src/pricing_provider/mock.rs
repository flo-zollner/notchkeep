...
impl MockProvider {
    ...
    pub fn with_symbol(mut self, isin: &str, symbol: &str) -> Self {
        self.symbol_for_isin
            .insert(isin.to_string(), symbol.to_string());
        self
    }
...
impl PriceProvider for MockProvider {
    fn fetch_quote<'a>(
        ...
        Box::pin(async move {
            self.quotes
                .get(symbol)
                .cloned()
                .ok_or_else(|| ProviderError::NotFound(format!("quote for {symbol}")))
        })
    }
    ...
    fn resolve_symbol<'a>(
        ...
        Box::pin(async move { Ok(self.symbol_for_isin.get(isin).cloned()) })
    }
}
    ...
    fn fetch_eur_rate<'a>(
        ...
        Box::pin(async move {
            ...
            self.fx_rates
                .get(&key)
                .copied()
                .ok_or_else(|| ProviderError::NotFound(format!("fx rate for {foreign_currency}")))
        })
    }
}
...
mod tests {
    ...
    async fn mock_history_filters_range() {
        let m = MockProvider::new().with_history(
            "AAPL",
            vec![
                PricePoint {
                    date: d("2026-01-15"),
                    close_micro: 150_000_000,
                },
                PricePoint {
                    date: d("2026-02-15"),
                    close_micro: 160_000_000,
                },
                PricePoint {
                    date: d("2026-03-15"),
                    close_micro: 170_000_000,
                },
            ],
        );
        let hist = m
            .fetch_history("AAPL", d("2026-02-01"), d("2026-02-28"))
            .await
            .unwrap();
    ...
    async fn mock_resolve_symbol_lookup() {
        ...
        assert_eq!(
            m.resolve_symbol("US0378331005").await.unwrap(),
            Some("AAPL".to_string())
        );
